...
public class LancamentoSimplesForm extends JDialog implements ActionListener {
    ...
    public void salvar() {
        ...
        try {
            if (this.cbCategoria.getSelectedIndex() >= 0)
                this.model.setCategoria(this.categorias.get(this.cbCategoria
                                .getSelectedIndex()));
            this.model.setDescricao(this.txDescricao.getText().trim());

            if (!this.txData.getText().isEmpty())
                this.model.setData((Date) this.df.stringToValue(this.txData
                                .getText()));
            else
                this.model.setData(null);

            Number valor = (Number) this.nf.stringToValue(this.txValor
                            .getText());
    ...
    protected void doData() {
        ...
        this.nf = new NumberFormatter(NumberFormat.getNumberInstance());
        ...
        try {
            if (this.model.getNumero() > 0)
                this.txNumero.setText(this.nf.valueToString(this.model
                                .getNumero()));

            if (this.model.getCategoria() != null)
                this.cbCategoria.setSelectedIndex(this.categorias
                                .indexOf(this.model.getCategoria()));
            else if (!this.categorias.isEmpty())
                this.cbCategoria.setSelectedIndex(0);

            this.txDescricao.setText(String
                            .valueOf(this.model.getDescricao() == null ? "" : this.model
                                            .getDescricao()));

            if (this.model.getData() != null)
                this.txData.setText(this.df.valueToString(this.model.getData()));
            else
                this.txData.setText(this.df.valueToString(GregorianCalendar
                                .getInstance().getTime()));
